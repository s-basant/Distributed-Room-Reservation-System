...
public class StudentClient {
	...
	public static StudentInterface connectToServer(String[] args){
	    ...
	    try {
	        ...
	        st = (StudentInterface) ncRef.resolve_str(SERVER_NAME+"_student_service");
	        return st;
	...
	private static void changeSlot() {
		...
		String roomNumber,campusName,success;
		roomNumber = campusName = success = "";
		...
		try{
		   success = remoteReference.changeReservation(bookingId,campusName,roomNumber,timeslot,studentID);		   
	...
	private static void cancelSlot() {
		...
		try {
			boolean success;
			success = remoteReference.cancelBooking(cancelId,studentID);
	...
	private static void bookSlot() {
		...
		try{
		   success = remoteReference.bookRoom(campusName,studentID,roomNumber, date, timeslot);		   
		   if(success.equals("fail")){
			  print("Failed to book a room");   
		   }else if(success.equals("overbooked")){
			  print("You can book only three slots a week"); 
	...
	private static void viewAvailbleSlots() {
		...
		try {
		  String result= remoteReference.getAvailableTimeSlot(date);
		  print(result);		    
		} catch (Exception e) {
			e.printStackTrace();
		}finally{
