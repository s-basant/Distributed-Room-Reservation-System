...
public class StudentImpl extends StudentInterfacePOA{
    ...
    LinkedHashMap<String, Integer> REMOTE_PORTS = new LinkedHashMap<String, Integer>() {{
        put("dvl",8090);
    	put("kkl",8091);
        put("wst",8092);
    }};
	...
	public String bookRoom(String campusName,String studentID, String roomNumber, String date, String timeslot){		
		...
		if(roomRecord.serverName.equals(campusName.toLowerCase())){
		    ...
		    System.out.println("inside the impl "+tmp);
		    System.out.println("room number "+roomNumber+" date "+date+" timeslot"+timeslot);
		}else{
	...
	public String getAvailableTimeSlot(String date) {		
		...
		if(isUDP && GlobalFlags.isByzantinError && roomRecord.serverName.equalsIgnoreCase("dvl")){
			return "DVL: 33";
		}
		...
		for(String k : REMOTE_PORTS.keySet()){
			if(!k.equalsIgnoreCase(roomRecord.serverName)){
				Integer[] f = {REMOTE_PORTS.get(k)};
				
				contactOtherServers("getAvailabeSlot:"+date,f);
